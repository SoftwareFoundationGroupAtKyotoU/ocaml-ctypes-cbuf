...
let rec format_typ' :
  ...
  fun t k context fmt ->
    match t with
    | Void -> fprintf fmt "void%t" (k `nonarray)
    | Primitive p ->
...
and format_fn' :
      'a. 'a fn -> (Format.formatter -> unit) -> Format.formatter -> unit =
  let rec gather : type a. a fn -> boxed_typ list * boxed_typ = function
    | Returns ty -> ([], BoxedType ty)
    | Function (Void, fn) -> gather fn
    | Function (p, fn) ->
        let ps, r = gather fn in
        (BoxedType p :: ps, r)
  in
  fun fn k fmt ->
    let ps, BoxedType r = gather fn in
    format_typ' r
      (fun _context fmt -> format_parameter_list ps k fmt)
      `nonarray fmt

let format_name ?name fmt =
  match name with Some name -> Format.fprintf fmt " %s" name | None -> ()
