...
let rec sizeof : type a. a typ -> int = function
  ...
  | Struct { spec = Complete { size; _ }; _ } -> size
  | Union { uspec = None; _ } -> raise IncompleteType
  | Union { uspec = Some { size; _ }; _ } -> size
...
let rec alignment : type a. a typ -> int = function
  ...
  | Struct { spec = Incomplete _; _ } -> raise IncompleteType
  | Struct { spec = Complete { align; _ }; _ } -> align
  | Union { uspec = None; _ } -> raise IncompleteType
...
let ( @-> ) f t =
  if not (passable f) then raise (Unsupported "Unsupported argument type")
  else Function (f, t)
...
let typedef old name =
  view
    ~format_typ:(fun k fmt -> Format.fprintf fmt "%s%t" name k)
...
let bigarray_ :
 ...
 fun spec dims kind l ->
  ...
  | Array3 ->
      let d1, d2, d3 = dims in
      Bigarray (Ctypes_bigarray.bigarray3 d1 d2 d3 kind l)

let bigarray spec c k = bigarray_ spec c k Bigarray_compat.c_layout

let fortran_bigarray spec c k =
  bigarray_ spec c k Bigarray_compat.fortran_layout

let returning v =
  if not (passable v) then raise (Unsupported "Unsupported return type")
  else Returns v
