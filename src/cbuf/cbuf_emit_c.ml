...
let cvar_name = function `Local (name, _) | `Global { name; _ } -> name
...
let rec camlxParam fmt args =
  match args with
  | [] -> ()
  | [ x1 ] -> fprintf fmt "@[CAMLxparam1@;(%s)@];" x1
  | [ x1; x2 ] -> fprintf fmt "@[CAMLxparam2@;(%s,@;%s)@];" x1 x2
  | [ x1; x2; x3 ] -> fprintf fmt "@[CAMLxparam3@;(%s,@;%s,@;%s)@];" x1 x2 x3
  | [ x1; x2; x3; x4 ] ->
...
let camlParam fmt args =
  ...
  | [ x1 ] -> fprintf fmt "@[CAMLparam1@;(%s)@];" x1
  | [ x1; x2 ] -> fprintf fmt "@[CAMLparam2@;(%s,@;%s)@];" x1 x2
  | [ x1; x2; x3 ] -> fprintf fmt "@[CAMLparam3@;(%s,@;%s,@;%s)@];" x1 x2 x3
  | [ x1; x2; x3; x4 ] ->
...
let cast_unnecessary : ty -> cexp -> bool =
  let rec harmless l r =
    match (l, r) with
    | Ty (Pointer Void), Ty (Pointer _) -> true
    | Ty (View { ty; _ }), t -> harmless (Ty ty) t
    | t, Ty (View { ty; _ }) -> harmless t (Ty ty)
...
let rec cexp fmt : cexp -> unit = function
  ...
  | `Addr (`Global { name; _ }) | `Addr (`Local (name, _)) ->
      fprintf fmt "@[&@[%s@]@]" name

let rec clvalue fmt : clvalue -> unit = function
  | #cvar as x -> cvar fmt x
  | `Index (lv, i) -> fprintf fmt "@[@[%a@]@[[%a]@]@]" clvalue lv cexp i
  | `Field (lv, f) -> fprintf fmt "@[@[%a@]@[.%s@]@]" clvalue lv f
  | `PointerField (lv, f) -> fprintf fmt "@[@[%a@]@[->%s@]@]" clvalue lv f

let camlop fmt : camlop -> unit = function
  | `CAMLparam0 -> Format.fprintf fmt "CAMLparam0()"
  | `CAMLlocalN (e, c) ->
      Format.fprintf fmt "CAMLlocalN(@[%a@],@ @[%a@])" cexp e cexp c
...
let rec ceff fmt : ceff -> unit = function
  ...
  | `App ({ fname; _ }, es) ->
      ...
      List.iteri
        ...
        es;
      fprintf fmt ")@]@]"
  | `Index (e, i) -> fprintf fmt "@[@[%a@]@[[%a]@]@]" ceff e cexp i
  | `Deref e -> fprintf fmt "@[*@[%a@]@]" cexp e
  | `DerefField (e, f) -> fprintf fmt "@[@[%a@]->%s@]" cexp e f

let rec ccomp fmt : ccomp -> unit = function
  | #cexp as e when Type_C.cexp e = Ty Void -> fprintf fmt "@[return@];"
  | #cexp as e -> fprintf fmt "@[<2>return@;@[%a@]@];" cexp e
  | #ceff as e -> fprintf fmt "@[<2>return@;@[%a@]@];" ceff e
  | `CAMLparam (xs, c) -> fprintf fmt "@[%a;@]@ %a" camlParam xs ccomp c
  | `Return (Ty Void, _) -> fprintf fmt "@[return@];"
  | `Return (Ty _ty, e) -> fprintf fmt "@[<2>return@;@[%a@]@];" cexp e
  | `CAMLreturnT (Ty Void, _) -> fprintf fmt "@[CAMLreturn0@];"
  | `CAMLreturnT (Ty ty, e) ->
      fprintf fmt "@[<2>CAMLreturnT(@[%a@],@;@[%a@])@];"
        (fun t -> Ctypes.format_typ t)
        ty cexp e
  | `Let (xe, `Cast (ty, (#cexp as e'))) when cast_unnecessary ty e' ->
      ccomp fmt (`Let (xe, e'))
  | `Let ((`Local (x, _), e), `Local (y, _)) when x = y ->
      ccomp fmt (e :> ccomp)
  | `Let ((`Local (_name, Ty Void), e), s) ->
      fprintf fmt "@[%a;@]@ %a" ceff e ccomp s
  | `Let ((`Local (name, Ty (Struct { tag; _ })), e), s) ->
      fprintf fmt "@[struct@;%s@;%s@;=@;@[%a;@]@]@ %a" tag name ceff e ccomp s
  | `Let ((`Local (name, Ty (Union { utag; _ })), e), s) ->
      fprintf fmt "@[union@;%s@;%s@;=@;@[%a;@]@]@ %a" utag name ceff e ccomp s
  | `Let ((`Local (name, Ty ty), e), s) ->
      fprintf fmt "@[@[%a@]@;=@;@[%a;@]@]@ %a" (Ctypes.format_typ ~name) ty ceff
        e ccomp s
  | `LetConst (`Local (x, _), `Int c, s) ->
      fprintf fmt "@[enum@ {@[@ %s@ =@ %s@ };@]@]@ %a" x
        (Signed.SInt.to_string c) ccomp s
  | `LetAssign (lv, e, c) ->
