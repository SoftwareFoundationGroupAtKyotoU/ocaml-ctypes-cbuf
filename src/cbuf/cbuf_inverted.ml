...
let format_enum_values fmt infos =
  List.iter
    (fun (Fn ({ fn_name; _ }, _)) -> Format.fprintf fmt "@[fn_%s,@]@ " fn_name)
    infos
...
let c_function fmt (Fn ({ fn_name; fn_runtime_lock }, fn)) : unit =
  Cbuf_generate_c.inverse_fn ~stub_name:fn_name ~runtime_lock:fn_runtime_lock
    fmt fn
...
let write_enum_declaration fmt (Enum (constants, ty)) =
  ...
  List.iteri
    (fun i (name, value) ->
      ...
      if i < last then Format.fprintf fmt "@[%s@ =@ %Ld,@]@\n" name value
...
let gen_ml fmt register (infos : fn_info list) : unit =
  ...
  Format.fprintf fmt "module CI = Cbuf_internals@\n@\n";
  ...
  ListLabels.iter infos ~f:(fun (Fn ({ fn_name; _ }, fn)) ->
      Cbuf_generate_ml.constructor_decl ~errno:`Ignore_errno
        (Printf.sprintf "Fn_%s" fn_name)
        fn fmt);
  Format.fprintf fmt "@\n";
  Format.fprintf fmt
    "@[<h>external register_value : 'a name -> 'a fn -> unit =@\n\
     ...
     @\n"
    register;
  Format.fprintf fmt "@[<h>let internal : ";
  Format.fprintf fmt
    "@[type a b.@ @[?runtime_lock:bool -> string -> (a -> b) Ctypes.fn -> (a \
     -> b) -> unit@]@]@ =@\n";
  ...
  ListLabels.iter infos ~f:(fun (Fn ({ fn_name; _ }, fn)) ->
      Cbuf_generate_ml.inverse_case ~register_name:"register_value"
        ~constructor:(Printf.sprintf "Fn_%s" fn_name)
        fn_name fmt fn);
  Format.fprintf fmt
