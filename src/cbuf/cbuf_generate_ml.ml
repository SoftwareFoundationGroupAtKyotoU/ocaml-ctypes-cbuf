...
let rec ml_typ_of_return_typ : type a. a typ -> ml_type = function
  ...
  | Primitive p ->
      `Ident
        (Cbuf_public_name.ident_of_ml_prim (Ctypes_primitive_types.ml_prim p))
...
let static_con c args = `Con (Cbuf_path.path_of_string ("CI." ^ c), args)
let local_con c args = `Con (Cbuf_path.path_of_string c, args)
...
let rec pattern_and_exp_of_typ :
 ...
 fun ~concurrency ~errno typ e pol binds ->
  ...
  | Primitive p ->
      let id =
        Cbuf_public_name.constructor_cident_of_prim ~module_name:"CI" p
      in
            ...
            ( `Ident use_value,
              `Tuple
                (ListLabels.map args ~f:(fun x ->
                     `Ident (Cbuf_path.path_of_string x))) ),
