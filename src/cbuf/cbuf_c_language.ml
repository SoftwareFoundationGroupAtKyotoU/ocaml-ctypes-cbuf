...
type ty = Ty : _ typ -> ty | BufTy : _ cbuffers -> ty
...
type cexp = [ cconst | clocal | `Cast of ty * cexp | `Addr of cvar ]
...
type camlop = [ `CAMLparam0 | `CAMLlocalN of cexp * cexp | `CAMLdrop ]
...
module Type_C = struct
  let cexp : cexp -> ty = function
    | `Int _ -> Ty sint
    | `Local (_, ty) -> ty
    | `Cast (Ty ty, _) -> Ty ty
    | `Cast (BufTy buf, _) -> BufTy buf
    | `Addr (`Global { typ = Ty ty; _ }) -> Ty (Pointer ty)
    | `Addr (`Global { typ = BufTy buf; _ }) -> BufTy buf
    | `Addr (`Local (_, Ty ty)) -> Ty (Pointer ty)
    | `Addr (`Local (_, BufTy buf)) -> BufTy buf

  let camlop : camlop -> ty = function
    | `CAMLparam0 | `CAMLlocalN _ | `CAMLdrop -> Ty Void
  ...
  and field_ceff : ceff -> fieldname -> ty =
   fun e f ->
    ...
    | Ty t ->
        Cbuf_errors.internal_error
          "accessing a field %s in an expression of type %s, which is not a \
  ...
  and lookup_field : type s a. string -> a typ -> s boxed_field list -> ty =
   fun f ty fields ->
    match fields with
    | [] ->
        Cbuf_errors.internal_error "field %s not found in struct %s" f
  ...
  let rec ccomp : ccomp -> ty = function
    ...
    | `Let (_, c) | `LetConst (_, _, c) -> ccomp c
    | `LetAssign (_, _, c) -> ccomp c
    | `CAMLreturnT (ty, _) -> ty
    | `Return (ty, _) -> ty
end

let value : [ `value ] abstract typ =
...
module Unchecked_function_types = struct
...
let prim_prj : type a. a Ctypes_primitive_types.prim -> _ =
  ...
  | Complex32 ->
      reader "ctypes_float_complex_val" (value @-> returning complex32)
  | Complex64 ->
      reader "ctypes_double_complex_val" (value @-> returning complex64)
  | Complexld ->
      reader "ctypes_ldouble_complex_val" (value @-> returning complexld)
...
let prim_inj : type a. a Ctypes_primitive_types.prim -> _ =
  ...
  | Complex32 ->
      conser "ctypes_copy_float_complex" (complex32 @-> returning value)
  | Complex64 ->
      conser "ctypes_copy_double_complex" (complex64 @-> returning value)
  | Complexld ->
      conser "ctypes_copy_ldouble_complex" (complexld @-> returning value)
