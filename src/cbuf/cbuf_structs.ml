...
let cstring s =
  ...
  let mappings = [ (Str.regexp "\"", "\\\""); (Str.regexp "\n", "\\n") ] in
  let escaped =
...
let cepilogue = [ "  return 0;"; "}" ]
let mlprologue = [ "include Ctypes"; "let lift x = x"; "open Ctypes_static" ]
...
let printf1 fmt s v =
  Format.fprintf fmt "@[ctypes_printf@[(%s,@ %t);@]@]@\n" (cstring s) v

(* [printf2 fmt s u v] writes the call [printf(s, u, v);] on [fmt]. *)
let printf2 fmt s u v =
  Format.fprintf fmt "@[ctypes_printf@[(%s,@ %t,@ %t);@]@]@\n" (cstring s) u v
...
let write_seal fmt specs =
  let case = function
    | `Struct (tag, typedef) ->
        ...
        puts fmt
          (Printf.sprintf
             "  | Struct ({ tag = %S; spec = Incomplete _; _ } as s') ->" tag);
    ...
    | `Union (tag, typedef) ->
        ...
        puts fmt
          (Printf.sprintf "  | Union ({ utag = %S; uspec = None; _ } as s') ->"
             tag);
        printf2 fmt "    s'.uspec <- Some { size = %zu; align = %zu }\n" usize
          ualign
    | `Other -> raise (Unsupported "Sealing a non-structured type")
  in
  cases fmt specs
...
let rec ml_pat_and_exp_of_typ : type a. a typ -> string * string =
 fun ty ->
  ...
  | Ctypes_static.View { Ctypes_static.ty; _ } ->
      ...
      let p' =
        Printf.sprintf "Ctypes_static.View { Ctypes_static.read = %s; ty = %s }"
          x p
      and e' = Printf.sprintf "(%s (%s))" x e in
      (p', e')
  | Ctypes_static.Primitive p ->
      let pat =
        Format.asprintf "Ctypes_static.Primitive %a" Cbuf_path.format_path
...
let write_consts fmt consts =
  let case = function
    | name, Ctypes_static.BoxedType ty ->
        let p, e = ml_pat_and_exp_of_typ ty in
        Format.fprintf fmt "{@[<v 2>@\n";
        ...
        printf1 fmt (Format.asprintf "  | %s, %S ->@\n    %s\n" p name e)
...
let write_enums fmt enums =
  let case (name, typedef) =
    printf1 fmt
      (Format.sprintf
         "  | %S -> \n\
         \    Cbuf_internals.build_enum_type %S Ctypes_static.%%s ?typedef \
          ?unexpected alist\n"
         name name) (fun fmt ->
        Format.fprintf fmt
...
let gen_c () =
  ...
  let m =
    (module struct
      ...
      let rec field' :
          type a s r. string -> s typ -> string -> a typ -> (a, r) field =
       fun structname s fname ftype ->
        match s with
        | Struct { tag; _ } ->
        ...
        | Union { utag; _ } ->
            ...
            { ftype; foffset = -1; fname }
        | View { ty; _ } -> field' structname ty fname ftype
        | _ -> raise (Unsupported "Adding a field to non-structured type")
      ...
      let constant name ty =
        consts := (name, Ctypes_static.BoxedType ty) :: !consts

      let enum name ?(typedef = false) ?unexpected _alist =
        let _ = unexpected in
        let () = enums := (name, typedef) :: !enums in
        let format_typ k fmt =
          Format.fprintf fmt "%s%s%t" (if typedef then "" else "enum ") name k
        in
        (* a dummy value of type 'a typ, mostly unusable *)
        view void ~format_typ
