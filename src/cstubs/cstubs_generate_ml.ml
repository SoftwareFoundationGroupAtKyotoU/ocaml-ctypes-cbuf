...
module Emit_ML : sig
  ...
  *)
  ...
  let rec ml_type arrow_parens fmt t =
    ...
    | ArrowParens, `Fn (t, t') ->
        fprintf fmt "@[(%a@ ->@ %a)@]" (ml_type ArrowParens) t
          (ml_type NoArrowParens) t'
    | NoArrowParens, `Fn (t, t') ->
        fprintf fmt "@[%a@ ->@]@ %a" (ml_type ArrowParens) t
          (ml_type NoArrowParens) t'
    | _, `Pair (t, t') ->
        fprintf fmt "@[(%a@ *@ %a)@]" (ml_type NoArrowParens) t
          (ml_type NoArrowParens) t'
  ...
  let args fmt xs = List.iter (fprintf fmt "%s@ ") xs
  ...
  let rec ml_exp appl_parens fmt (e : ml_exp) =
    ...
    | ApplParens, `Appl (f, p) ->
        fprintf fmt "@[(%a@;<1 2>%a)@]" (ml_exp NoApplParens) f
          (ml_exp ApplParens) p
    | NoApplParens, `Appl (f, p) ->
        fprintf fmt "@[%a@ %a@]" (ml_exp NoApplParens) f (ml_exp ApplParens) p
    | ApplParens, `MakePtr (t, e) ->
        fprintf fmt "(@[<hov 2>CI.make_ptr@ %a@ %a)@]" (ml_exp ApplParens) t
          (ml_exp ApplParens) e
    | NoApplParens, `MakePtr (t, e) ->
        fprintf fmt "@[<hov 2>CI.make_ptr@ %a@ %a@]" (ml_exp ApplParens) t
          (ml_exp ApplParens) e
    | ApplParens, `MakeFunPtr (t, e) ->
        fprintf fmt "(@[<hov 2>CI.make_fun_ptr@ %a@ %a)@]" (ml_exp ApplParens) t
          (ml_exp ApplParens) e
    | NoApplParens, `MakeFunPtr (t, e) ->
        fprintf fmt "@[<hov 2>CI.make_fun_ptr@ %a@ %a@]" (ml_exp ApplParens) t
          (ml_exp ApplParens) e
    | ApplParens, `MakeStructured (t, e) ->
        fprintf fmt "(@[<hov 2>CI.make_structured@ %a@ %a)@]"
          (ml_exp ApplParens) t (ml_exp ApplParens) e
    | NoApplParens, `MakeStructured (t, e) ->
        fprintf fmt "@[<hov 2>CI.make_structured@ %a@ %a@]" (ml_exp ApplParens)
          t (ml_exp ApplParens) e
    | _, `Fun (xs, e) ->
        fprintf fmt "(@[<1>fun@ %a->@ %a)@]" args xs (ml_exp NoApplParens) e
    | _, `Tuple es -> fprintf fmt "(@[%a)@]" tuple_elements es
    | _, `Seq (e1, e2) ->
        fprintf fmt "(@[%a;@ %a)@]" (ml_exp NoApplParens) e1
          (ml_exp NoApplParens) e2
  ...
  let extern fmt { ident; typ; primname; primname_byte; attributes } =
    fprintf fmt "@[<hov 2>@[external@ %s@]@ @[<h 1>:@ @[%a@]@]@ " ident
      ml_external_type typ;
    fprintf fmt "@[=@ @[@[%a@]@[%S@]@ %a@]@]@]@." primname_opt primname_byte
      primname attrs attributes
end

let arity : ml_external_type -> int = fun (`Prim (args, _)) -> List.length args
let max_byte_args = 5

let byte_stub_name : string -> ml_external_type -> string option =
 fun name t ->
  let arity = arity t in
  if arity > max_byte_args then Some (Printf.sprintf "%s_byte%d" name arity)
  else None
...
let fatfunptr =
  `Appl (path_of_string "CI.fatfunptr", [ `Ident (path_of_string "_") ])

(* These functions determine the type that should appear in the extern
   signature *)
let rec ml_typ_of_return_typ : type a. a typ -> ml_type = function
  | Void -> `Ident (path_of_string "unit")
  | Primitive p ->
      `Ident
        (Cstubs_public_name.ident_of_ml_prim (Ctypes_primitive_types.ml_prim p))
  ...
  | Array _ as a ->
      internal_error "Unexpected array type in the return type: %s"
        (Ctypes.string_of_typ a)
  | Bigarray _ as a ->
      internal_error "Unexpected bigarray type in the return type: %s"
        (Ctypes.string_of_typ a)
  | OCaml String ->
...
let rec ml_typ_of_arg_typ : type a. a typ -> ml_type = function
  ...
  | Array _ as a ->
      internal_error "Unexpected array in an argument type: %s"
        (Ctypes.string_of_typ a)
  | Bigarray _ as a ->
      internal_error "Unexpected bigarray in an argument type: %s"
        (Ctypes.string_of_typ a)
  | OCaml String ->
      `Appl (path_of_string "CI.ocaml", [ `Ident (path_of_string "string") ])
  | OCaml Bytes ->
      `Appl (path_of_string "CI.ocaml", [ `Ident (path_of_string "bytes") ])
  | OCaml FloatArray ->
      `Appl
        ( path_of_string "CI.ocaml",
          [
...
let static_con c args = `Con (Ctypes_path.path_of_string ("CI." ^ c), args)
let local_con c args = `Con (Ctypes_path.path_of_string c, args)
...
let map_result ~concurrency ~errno f e =
  ...
  | #non_lwt, `Ignore_errno, `Appl x -> `Appl (`Ident (path_of_string x), e)
  | _, _, `MakePtr x ->
  ...
  | _, _, `Appl x -> map_result (`Ident (path_of_string x)) e
...
let rec pattern_and_exp_of_typ :
    ...
    (ml_pat * ml_exp) list ->
    pattern_exp_return =
 fun ~concurrency ~errno typ e pol binds ->
  match typ with
  | Void -> (static_con "Void" [], None, binds)
  | Primitive p ->
      let id =
        Cstubs_public_name.constructor_cident_of_prim ~module_name:"CI" p
      in
  ...
  | Pointer _ -> (
      ...
      | In ->
          ...
          let x = fresh_var () in
          ( pat,
            Some (`Ident (path_of_string x)),
            binds @ [ (static_con "CPointer" [ `Var x ], e) ] )
      | Out ->
          let x = fresh_var () in
  ...
  | Funptr _ -> (
      ...
      | In ->
          ...
          ( pat,
            Some (`Ident (path_of_string x)),
            binds @ [ (static_con "Static_funptr" [ `Var x ], e) ] )
      | Out ->
          let x = fresh_var () in
  ...
  | Struct _ -> (
      ...
      | Out ->
          ...
          ( pat,
            Some (map_result ~concurrency ~errno (`MakeStructured x) e),
            binds ))
  | Union _ -> (
      match pol with
      | In ->
  ...
  | View { ty; _ } -> (
      ...
      | In ->
          ...
          let ((p, None, binds), e | (p, Some e, binds), _) =
            (pattern_and_exp_of_typ ~concurrency ~errno ty e pol binds, e)
          in
          let pat =
            static_con "View"
              [
                `Record
                  ( [
                      (path_of_string "CI.ty", p);
                      (path_of_string "write", `Var x);
                    ],
                    `Etc );
              ]
          in
          ( pat,
            Some (`Ident (Ctypes_path.path_of_string y)),
            (`Var y, e) :: binds )
      | Out ->
          let ((p, None, binds), e | (p, Some e, binds), _) =
            (pattern_and_exp_of_typ ~concurrency ~errno ty e pol binds, e)
  ...
  | OCaml ty -> (
      match (pol, ty) with
      | In, String ->
          (static_con "OCaml" [ static_con "String" [] ], None, binds)
      | In, Bytes -> (static_con "OCaml" [ static_con "Bytes" [] ], None, binds)
      | In, FloatArray ->
          (static_con "OCaml" [ static_con "FloatArray" [] ], None, binds)
      | Out, String ->
          Ctypes_static.unsupported
            "cstubs does not support OCaml strings as return values"
      | Out, Bytes ->
...
let rec pattern_of_typ : type a. a typ -> ml_pat = function
  ...
  | Pointer _ -> static_con "Pointer" [ `Underscore ]
  | Funptr _ -> static_con "Funptr" [ `Underscore ]
  | Struct _ -> static_con "Struct" [ `Underscore ]
  | Union _ -> static_con "Union" [ `Underscore ]
  | View { ty; _ } ->
      static_con "View"
        [ `Record ([ (path_of_string "CI.ty", pattern_of_typ ty) ], `Etc) ]
  | Array (_, _) -> static_con "Array" [ `Underscore; `Underscore ]
  | Bigarray _ -> static_con "Bigarray" [ `Underscore ]
  | OCaml String ->
      Ctypes_static.unsupported
...
let run_exp ~concurrency exp =
  match concurrency with
  | #non_lwt -> exp
  | `Lwt_jobs -> `Appl (`Ident lwt_unix_run_job, exp)
  | `Lwt_preemptive ->
      `Appl (`Appl (`Ident lwt_preemptive_detach, `Fun ([ "_" ], exp)), `Unit)
  ...
  | Function (f, t) -> (
      ...
      | fpat, None, binds ->
          let { exp; args; trivial; pat = tpat; binds } =
            wrapper_body ~concurrency ~errno t
              (`Appl (exp, `Ident (path_of_string x)))
              pol binds
          in
          {
            exp;
      ...
      | fpat, Some exp', binds ->
          let { exp; args = xs; trivial = _; pat = tpat; binds } =
            wrapper_body ~concurrency ~errno t (`Appl (exp, exp')) pol binds
          in
          {
            exp;
            args = x :: xs;
    ...
    ( [ x ],
      `Seq
        ( `Appl
            ( `Ident use_value,
              `Tuple
                (ListLabels.map args ~f:(fun x ->
                     `Ident (Ctypes_path.path_of_string x))) ),
...
let wrapper :
 ...
 fun ~concurrency ~errno id fn f pol ->
  let p =
    wrapper_body ~concurrency ~errno fn (`Ident (path_of_string f)) pol []
  in
  ...
  | { trivial = true; pat; args; binds; _ }, #lwt ->
      let exp : ml_exp =
        List.fold_left
          (fun f p -> `Appl (f, `Ident (path_of_string p)))
          (`Ident id) args
      in
      ( pat,
        `Fun
          ( args,
            let_bind binds
              (`Appl
                ( `Ident box_lwt,
                  `Appl
                    ( `Appl (`Ident lwt_bind, run_exp ~concurrency exp),
                      return_result
                        ~args:(args @ pats_bound_vars (List.map fst binds)) ) ))
          ) )
...
let inverse_case ~register_name ~constructor name fmt fn : unit =
  let p, e =
    wrapper ~concurrency:`Sequential ~errno:`Ignore_errno (path_of_string "f")
      fn "f" Out
  in
  Format.fprintf fmt "|@[ @[%a, %S@] -> %s %s (%a)@]@\n"
    Emit_ML.(ml_pat NoApplParens)
    p name register_name constructor
