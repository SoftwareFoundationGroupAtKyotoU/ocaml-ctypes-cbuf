...
let gen_c ~concurrency ~errno prefix fmt : (module FOREIGN') =
  (module struct
    ...
    let foreign cname fn =
      Cstubs_generate_c.fn ~concurrency ~errno ~cname
        ~stub_name:(var prefix cname) fmt fn
...
let write_foreign ~concurrency ~errno fmt bindings val_bindings =
  ...
  ListLabels.iter bindings ~f:(fun (Bind (stub_name, external_name, fn)) ->
      Cstubs_generate_ml.case ~concurrency ~errno ~stub_name ~external_name fmt
        fn);
...
let gen_ml ~concurrency ~errno prefix fmt : (module FOREIGN') * (unit -> unit) =
  ...
  ( (module struct
      ...
      let foreign cname fn =
        ...
        Cstubs_generate_ml.extern ~concurrency ~errno ~stub_name:name
...
let write_c ?(concurrency = `Sequential) ?(errno = `Ignore_errno) fmt ~prefix
    (module B : BINDINGS) =
